...
| P0 | Finalize and attest to a data deletion and retention policy | Plaid remediation / questionnaire Q11 | Implemented in app as a public policy, but review cadence and legal alignment are still informal | Formal policy owner, review cadence, and operator procedure are documented |
| P0 | Enable consumer MFA before Plaid Link | Plaid remediation / questionnaire Q4 | Not implemented; there is no user auth system yet | Consumer authentication and MFA gate the app before bank-linking workflows |
| P0 | Require MFA on internal systems storing or processing consumer data | Plaid remediation / questionnaire Q5 | Partially dependent on external providers; not documented as enforced across all critical systems | MFA is mandatory on Plaid, hosting, source control, email, secrets, and admin tools |
| P0 | Secure data in transit with TLS 1.2+ in production | Plaid remediation / questionnaire Q6 | Depends on deployment; localhost development does not satisfy this | Production deployment enforces HTTPS/TLS 1.2+ with HSTS where appropriate |
...
| P0 | Implement vulnerability scanning | Plaid remediation / questionnaire Q8 | No formal recurring vulnerability scanning program is in place | Regular endpoint, dependency, and infrastructure scanning is operationalized |
| P0 | Define and enforce a vulnerability patch SLA | Plaid remediation | No documented SLA yet | Severity-based remediation SLAs are documented and followed |
| P1 | Implement periodic access reviews and audits | Plaid remediation / questionnaire Q3 | No formal access review process exists | Quarterly access review checklist and audit log are maintained |
| P1 | Create and maintain a defined access control policy | Plaid remediation / questionnaire Q3 | Partially covered in ISP, but no dedicated access control policy exists | Separate documented access control policy covers least privilege, approvals, reviews, and revocation |
| P1 | Implement centralized identity and access management | Plaid remediation | Not implemented | Critical systems are managed through centralized identity with role-based assignment |
| P1 | Implement automated de-provisioning / modification of access | Plaid remediation | Not implemented | Offboarding and role-change workflows remove or reduce access promptly |
| P1 | Implement zero trust access architecture for production systems | Plaid remediation | Not implemented | Access to production is brokered through identity-aware, least-privilege controls |
| P1 | Monitor end-of-life software and document EOL management | Plaid remediation | Not documented as a formal practice | Software inventory includes runtime/framework versions and EOL review checkpoints |
| P1 | Operationalize the Information Security Policy | Plaid remediation / questionnaire Q2 | A written ISP now exists, but supporting procedures are still light | Named owner, annual review, and linked procedures exist for access, incident response, patching, and retention |
